...
flags.DEFINE_string("test_fname", "../../csv_files/STT.csv", "The name of the test file.")
flags.DEFINE_string("train_fname", "fake_train.tsv",
                    "The name of the training file.")
flags.DEFINE_string("dev_fname", "fake_dev.tsv", "The name of the dev file.")
...
class InputExample(object):
  ...
  def __init__(self, guid, text, start_time, end_time, labels=None):
...
class DataProcessor(object):
  ...
  def _read_df(cls, input_file, data_type):
    ...
    if data_type == "test":
      names = ["start_time", "end_time", "text"]
    else:
      names = ["start_time", "end_time", "text", "labels"]
  ...
  def _create_examples(self, df, data_type):
    ...
    for (i, row) in df.iterrows():
      ...
      examples.append(InputExample(guid=guid, text=text, start_time=start_time, end_time=end_time,labels=labels))
    return examples
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, FLAGS.test_fname + ".predictions.tsv")
    output_labels = os.path.join(FLAGS.output_dir,
                                 "../../csv_files/STT_emotion_predictions.tsv")

    with tf.gfile.GFile(output_predict_file, "w") as writer:
      with tf.gfile.GFile(output_labels, "w") as writer2:
        writer.write("\t".join(all_emotions) + "\n")
        writer2.write("\t".join([
            "text", "start_time", "end_time", "emotion_1", "prob_1", "emotion_2", "prob_2", "emotion_3", "prob_3"
        ]) + "\n")
        ...
        for (i, prediction) in enumerate(result):
          ...
          writer2.write(str(predict_examples[i].start_time) + "\t" + str(predict_examples[i].end_time) + "\t" +
                        predict_examples[i].text + "\t" + "\t".join(pred_line) +
                        "\n")
          num_written_lines += 1
